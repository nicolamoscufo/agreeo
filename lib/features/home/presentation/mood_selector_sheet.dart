...
class _MoodSelectorSheetState extends ConsumerState<MoodSelectorSheet> {
  ...
  Future<void> _match() async {
    ...
    if (feeling.isEmpty && want.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Describe how you feel or want to feel.')),
      );
      return;
    }
    ...
    try {
      final movies = await ref
          ...
          .getMoviesByMood(feeling: feeling, wantToFeel: want);
      if (mounted) {
        setState(() {
          _results = movies;
          _loading = false;
      ...
      if (mounted) {
        setState(() {
          _error = 'Could not match your vibe. Try again.';
          _loading = false;
        });
      }
    }
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: AgText.caption.copyWith(height: 1.45, color: t.sub),
          ),
          ...
          if (!_searched) ...[
            ...
            _MoodField(
              controller: _feeling,
              hint: 'e.g. stressed, tired, down…',
            ),
            const SizedBox(height: 16),
            _Label('How do you want to feel after?'),
            const SizedBox(height: 8),
            _MoodField(
              controller: _wantToFeel,
              hint: 'e.g. relaxed, happy, scared…',
              focus: true,
            ),
  ...
  Widget _buildResults(AgreeoTokens t) {
        ...
        child: Column(
          children: [
            SizedBox(
              width: 44,
              height: 44,
              child: CircularProgressIndicator(color: t.gold, strokeWidth: 3.5),
            ),
    ...
    if (_results.isEmpty) {
      return AgStateCard(
        icon: AgIcons.film,
        title: 'No matches for your vibe',
        message: 'Adjust your inputs or pick another quick vibe.',
        actionLabel: 'Back to inputs',
        onAction: _reset,
      );
    }
    return Column(
      ...
      children: [
        Row(
          ...
          children: [
            Text('Your vibe matches', style: AgText.h3.copyWith(color: t.text)),
            Text(
              '${_results.length} titles found',
              style: AgText.label.copyWith(color: t.gold),
            ),
          ],
        ),
                    ...
                    AgPoster(
                      ...
                      onTap: () => Navigator.of(context).push(
                        MaterialPageRoute<void>(
                          builder: (_) =>
                              AgreeoMovieDetailsScreen(movieId: m.id),
                        ),
                      ),
                    ),
                    const SizedBox(height: 8),
...
class _Label extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(
      text,
      style: AgText.h4.copyWith(fontSize: 14.5, color: context.tokens.text),
    );
  }
}
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: focus ? t.red.withValues(alpha: 0.4) : t.line,
          width: focus ? 1.5 : 1,
        ),
      ),
      ...
      child: TextField(
        ...
        maxLines: 2,
        cursorColor: t.red,
        style: AgText.body.copyWith(color: t.text, height: 1.4),
